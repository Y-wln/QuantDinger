...
class SelfCheck:
    ...
    def _check_exchange(self) -> dict:
        ...
        try:
            import os
            gate_testnet = os.getenv("GATE_TESTNET", "").lower() == "true"
            bybit_testnet = os.getenv("BYBIT_TESTNET", "").lower() == "true"
            auto_exec = os.getenv("HERMES_AUTO_EXECUTE", "").lower() == "true"
            
            configured = []
            if os.getenv("BINANCE_API_KEY"): configured.append("binance")
            if os.getenv("EXCHANGE_API_KEY"): configured.append("generic")
            if gate_testnet: configured.append("gate(testnet)")
            if bybit_testnet: configured.append("bybit(testnet)")
            ...
            if configured:
                return {"status": "OK", "detail": f"{' + '.join(configured)}, auto_exec={auto_exec}"}
            return {"status": "NOT_CONFIGURED", "detail": "No exchange credentials"}
        except Exception as e:

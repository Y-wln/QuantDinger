...
def analyze_one(sym):
    try:
        ...
        bb_bw = bb.get('bandwidth', 50) if bb else 50
...
coins = ['BTCUSDT','ETHUSDT','SOLUSDT','DASHUSDT','INJUSDT','TAOUSDT']
for sym in coins:
    r = analyze_one(sym)
    if r and 'error' not in r:
        emoji = 'LONG' if r['signal']=='long' else ('SHORT' if r['signal']=='short' else 'WAIT')
        print(emoji, sym.replace('USDT','').ljust(6), 'score='+str(r['score']).rjust(4), 'price='+str(r['price']))
        print('  1hCVD='+str(r['cvd_1h']).rjust(5)+'%  15mCVD='+str(r['cvd_15m']).rjust(5)+'%  delta='+str(r['cv_delta']).rjust(5)+'%  RSI='+str(r['rsi'])+'  BB='+str(r['bb_bw'])+'%')
        print('  trend: 1h='+r['trend_1h']+' 15m='+r['trend_15m'])

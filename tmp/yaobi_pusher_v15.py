# yaobi_pusher.py v15.0 - v14 + pump-dump detection + micro-coin exhaust 1.5%
...
print("[YaobiPusher v15 FULL] starting...")
feishu_app_send("Yaobi Pusher v15 | pump-dump +micro-exhaust | gate>=3-confirms +OB-final +no-chase +choppy | " + datetime.now(BJT).strftime("%H:%M"), chat_id=CHAT_ID)
...
while True:
    try:
        ...
        # Stage 1: volatility pre-filter (v6: 0.8%鈫?.5%)
        def check_vol(sym):
            try:
        ...
        if candidates:
            def full_scan(sym_chg):
                ...
                try:
                    ...
                    # === v15: Pump/dump detection ===
                    is_pump, pump_type, pump_msg = detect_pump_dump(k5)
                    if is_pump and pump_type == "pump_long":
                        vote_dir = "short"; vote_count = 3.0
                        vr_list = []; vs_list = [pump_msg]
                    elif is_pump and pump_type == "pump_short":
                        vote_dir = "long"; vote_count = 3.0
                        vr_list = [pump_msg]; vs_list = []
                    else:
                        # VOTE
                    ...
                    # v15: exhaustion gate - micro coins 1.5%, others 3%
        ...
        if signals:
            ...
            lines = ["----------------", "  馃幆 濡栧竵鎵弿 V15 | %s" % t, "----------------"]
            
            longs = [s for s in signals if s["dir"] == "long"]
            shorts = [s for s in signals if s["dir"] == "short"]
            
            if longs:
                lines.append("  馃煝 鈹佲攣 鍋氬淇″彿 鈹佲攣")
                for s in longs[:5]:
                    sym = s["sym"].replace("USDT", "")
                    reason_str = " | ".join(s.get("reasons", [])[:3])
                    badges = ""
                    if s.get("is_flip"): badges += "馃毃缈昏浆 "
                    if s.get("streak", 1) >= 3: badges += "馃敟鎸佷箙"
                    ob = s.get("ob_imb", 0)
                    ob_str = " OB:%+d%%" % int(ob) if abs(ob) > 10 else ""
                    cross_badge = get_badge(sym, "long", "yaobi")
                    lines.append("    鍋氬 %-6s %3d鍒?$%s %s%s%s" % (sym, s["score"], s["price"], badges, cross_badge, ob_str))
                    if reason_str:
                        lines.append("      鈫?%s" % reason_str[:70])
            
            if shorts:
                lines.append("  馃敶 鈹佲攣 鍋氱┖淇″彿 鈹佲攣")
                for s in shorts[:5]:
                    sym = s["sym"].replace("USDT", "")
                    reason_str = " | ".join(s.get("reasons", [])[:3])
                    badges = ""
                    ...
                    lines.append("    鍋氱┖ %-6s %3d鍒?$%s %s%s%s" % (sym, s["score"], s["price"], badges, cross_badge, ob_str))
                    if reason_str:
                        lines.append("      鈫?%s" % reason_str[:70])
            ...
            print("[YaobiV15] pushed %d (L:%d S:%d) in %.1fs" % (len(signals), len(longs), len(shorts), time.time()-t0))
        else:
            print("[YaobiV15] no signals in %.1fs" % (time.time()-t0))
        
        elapsed = time.time() - t0
        time.sleep(max(10, 120 - elapsed))
    except Exception as e:
        print("[YaobiV15] error:", e)
        import traceback
        traceback.print_exc()
        time.sleep(30)

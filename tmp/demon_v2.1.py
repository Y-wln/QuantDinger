...
def detect(k5, k1=None):
    ...
    # Anti-falling-knife: 5 consecutive red candles = no long
    if long_score >= 40:
        red5 = sum(1 for i in range(-5, 0) if float(k5[i]["c"]) < float(k5[i]["o"]))
        if red5 >= 5:
            long_score = 0; long_reasons = ["FALLING_KNIFE(5red)"]

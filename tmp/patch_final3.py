...
# ===== PATCH 1: process_signals (L241-L292 -> 0-indexed 240-291) =====
...
lines = lines[:240] + new_ps.splitlines(True) + lines[292:]

# ===== PATCH 2: btc_vane before monitor_positions =====
mp = -1
for i, l in enumerate(lines):
    if l.strip().startswith("def monitor_positions"):
        mp = i; break

btc = """    def btc_vane(self):
        try:
...
"""
lines = lines[:mp] + btc.splitlines(True) + lines[mp:]

# ===== PATCH 3: timeout before SL check =====
sl = -1
for i, l in enumerate(lines):
    s = l.strip()
...
# Now replace: remove the original "if (direction==..." line and replace with timeout + elif version
# strategy: replace the original line with timeout + "                elif (direction==..."
orig_line = lines[sl]
indent = "                "
elif_line = indent + "el" + orig_line.lstrip()[2:]  # "if" -> "elif"
tmo_lines = tmo.splitlines(True)
tmo_lines[-1] = elif_line  # replace the "el" placeholder

lines = lines[:sl] + tmo_lines + lines[sl+1:]

#!/usr/bin/env python3
"""tracker_daemon.py v2 - Standalone tracking engine, zero coupling with strategy"""
...
def build_pipeline():
    ...
    for sig in data.get("signals", []):
        ...
        orig_score = extras.get("score_breakdown", {}).get("original", sig.get("score", 0))
        final_score = extras.get("final_score", sig.get("score", 0))
        ...
        breakdown = extras.get("score_breakdown", {})
        if breakdown:
            adj = {k: v for k, v in breakdown.items() if k != "original"}
            entry["scored"] = {
                "original": orig_score,
                "final": final_score,
                "adjustments": adj,
                "total_adjustment": sum(adj.values()) if adj else 0,
            }
...
def main():
    ...
    print("[tracker_daemon] v2 started - score breakdown tracking")
    ...
    while True:
        try:
            ...
            n_total = len(sigs)
            n_scored = sum(1 for s in sigs.values() if "scored" in s)
            ...
            print(f"  [{time.strftime('%H:%M')}] sigs:{n_total} conf:{n_confirm} scored:{n_scored} open:{n_opened} closed:{n_closed}")

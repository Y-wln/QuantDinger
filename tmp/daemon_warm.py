"""V2 Daemon: pre-warms kline cache, then runs full pipeline."""
...
# Pre-warm kline cache (single-threaded to avoid rate limits)
print("Pre-warming kline cache...")
orch = Orchestrator()
all_coins = list(set(orch.cfg["scan_coins"] + list(YAOBI_COINS)))
for i, sym in enumerate(all_coins):
    print(f"  [{i+1}/{len(all_coins)}] {sym}...", end=" ", flush=True)
    t0 = time.time()
    try:
        orch.kline_cache.get(sym, "4h", 300)
        orch.kline_cache.get(sym, "1h", 300)
        orch.kline_cache.get(sym, "5m", 50)
        orch.kline_cache.get(sym, "15m", 30)
        print(f"OK ({time.time()-t0:.1f}s)")
    except Exception as e:
        print(f"FAIL ({e})")
    time.sleep(0.5)  # rate limit safety

print("Cache warm. Starting pipeline...")

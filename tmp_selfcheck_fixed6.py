...
class SelfCheck:
    ...
    def _check_exchange(self) -> dict:
        """Check exchange connectivity via hermes QD bridge."""
        try:
            from app.services.hermes_strategies.hermes_qd_bridge import integrate_with_quantdinger
            result = integrate_with_quantdinger()
            exec_status = result.get("execution", "unknown")
            if exec_status in ("active", "simulation", "paper"):
                return {"status": "OK", "detail": f"QD bridge: {exec_status}"}
            elif exec_status == "no_credentials":
                return {"status": "NOT_CONFIGURED", "detail": "No exchange credentials (Gate.io testnet in DB)"}
            else:
                return {"status": "OK", "detail": f"QD bridge: {exec_status}"}
        except Exception as e:

...
class SafeJSONProvider(DefaultJSONProvider):
    ...
    1. NaN / Infinity 閳?null.  ``json.dumps`` (allow_nan=True) emits literal
       ``NaN`` / ``Infinity`` tokens which are **not** valid JSON per RFC 8259
       and crash ``JSON.parse()`` on the frontend.

    2. ``datetime`` 閳?UTC ISO 8601 (``...Z``).  Database columns are stored as
        ...
        for strategy_info in running_strategies:
            ...
            try:
                ...
                if success:
                    ...
                    # 婵″倹鐏夐幁銏狀槻婢惰精瑙﹂敍灞炬纯閺傜増鏆熼幑顔肩氨閻樿埖鈧椒璐焥topped閿涘矂浼╅崗宥囩摜閻ｃ儱顦╂禍?閸嶉潧妗?閻樿埖鈧?
                    try:
...
def create_app(config_name='default'):
    ...
    # CORS 閳?pin to specific origins instead of '*'. FRONTEND_URL accepts a
    ...
    #   - Android (capacitor 6, androidScheme="https") 閳?https://localhost
    #   - Android (capacitor 5 or scheme="http")      閳?http://localhost
    #   - iOS (capacitor 6, iosScheme="https")        閳?capacitor://localhost
    #   - iOS legacy (ionic://)                       閳?ionic://localhost
    #   - Cordova / file:// loaded apps               閳?null  (Origin: null)
    #
    ...
    # uses; see api/index.js 閳?`Authorization: Bearer ${token}`).
    CORS(
        app,

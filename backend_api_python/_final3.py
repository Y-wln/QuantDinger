...
print(f'Setup: {time.time()-t0:.1f}s ({bus.subscriber_count()} handlers)')

for s in strategies:
    sigs = s.generate(mercu)
    for sig in sigs:
        bus.emit(Event(EventType.SIGNAL_GENERATED, sig.to_dict(), source=s.name))
print(f'Generated+Tracked: {len(tracker._signals)} in {time.time()-t0:.1f}s')
...
for i, (sid, sig) in enumerate(tracker._signals.items()):
    ...
    if sig.direction == 'LONG': tracker.close_signal(sid, entry * mult)
    else: tracker.close_signal(sid, entry / mult)

stats = tracker.get_stats()
tracker.save()
print(f'WinRate: {stats["win_rate"]}%')
print(f'By source: {list(stats["by_source"].keys())}')
print(f'By indicator: {list(stats["by_indicator"].keys())}')

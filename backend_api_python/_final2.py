...
# Close signals (handle zero price)
for i, (sid, sig) in enumerate(tracker._signals.items()):
    entry = sig.entry_price if sig.entry_price > 0 else 1.0  # fallback
...
for ind, s in list(stats['by_indicator'].items())[:6]:
    print(f'  {ind}: {s["signals"]}x WR={s["win_rate"]}%')
print(f'By coin:')
for coin, s in stats['by_coin'].items():
    print(f'  {coin}: {s["total"]}x WR={s["win_rate"]}%')

tracker.save()
print(f'\nSaved. State file: OK')
print('ALL PASSED - No division errors, 8 signals fully tracked')
